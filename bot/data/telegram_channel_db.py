...
class telegram_db():
    ...
    async def add_telegram_channel(self, url: str, id: int):
        client = get_url_connection()
        info = client[self.name]
        telegram_channels_collection = info.telegram_channels
        try:
            if await telegram_db.get_telegram_channel_by_id(self ,id = id) is None:
    ...
    async def delete_telegram_channel(self, id: int):
    ...
    async def get_telegram_channel_by_id(self, id: int):
    ...
    async def get_telegram_channels_by_url(self, url: str):
    ...
    async def update_date_telegram_channel(self, id: int, date: datetime):
    ...
    async def get_telegramm_channels(self):
        ...
        try:
            ...
            async for channel in telegram_channels_collection.find():
                if self.name == "info":
                    ...
                    if channel.get("id") is not None:
                        telegram_channels[channel['id']] = (channel['link_channel'], channel['last_updated'], channel['last_send'], channel['key_words'], channel['bad_words'])
    ...
    async def update_telegram_channel_last_send(self, id:int, date: datetime):
    ...
    async def exists_new_update(self, id:int):
        log_db().send_info(f"Starting exists_new_update() function for channel ID {id}.")
        try:
            tg_channel = await telegram_db.get_telegram_channel_by_id(self= self , id = id)
    ...
    async def update_id_telegram_channel(self, id: int, id_new: int):
        log_db().send_info(f"Starting update_date_telegram_channel() function for channel ID {id}.")
    ...
    async def update_link_telegram_channel(self, id: int, link_new: int):
        log_db().send_info(f"Starting update_date_telegram_channel() function for channel ID {id}.")
    ...
    #work with documents 
    async def check_post_exist_in_telegram_channel(self, id: str, date: datetime):
    ...
    async def add_image_to_telegram_channel(self, id: str, url: str, date: datetime):
    ...
    async def create_new_telegram_channel_parsing(self, id: str, date: datetime, text: str, id_collection: int):
